...
exports.notApprovedProblems = function(req, res) {
    req.getConnection(function(err, connection) {
        if (err) {
            ...
            connection.query('SELECT Problems.Id, Problems.Title, Activities.Date FROM Problems JOIN Activities ON Problems.Id = Activities.Problems.Id WHERE Moderation = 0;', function(err, rows, fields) {
                if (err) {
                    console.error(err);
                    res.statusCode = 500;
                    res.send({
