...
connectionpool.getConnection(function(err, connection) {
		if (err) {
            ...
            jwt.verify(token, secret.secretToken, function(err, decoded) {
                ...
                connection.query('SELECT Problems.Title, Activities.Date FROM Problems JOIN Activities ON Problems.Id = Activities.Id WHERE Moderation IS NULL;', function(err, rows, fields) {
                    if (err) {
                        console.error(err);
                        res.statusCode = 500;
                        res.send({
